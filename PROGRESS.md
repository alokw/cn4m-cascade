...
| D-138 | **The Webhooks tab can edit a callback, and shows what each one is set to** | Reported 2026-09-12: settings typed into the tab "don't seem to stick". They never could. `PATCH /api/webhooks/{id}` has existed since Phase 5b and `api.webhooks.update` was in the client, but **the UI never called either** — `CallbackList` had `add` and `remove` and nothing else, and the global rows (the cn4m and Discord callbacks) rendered with no actions at all. What looked like editing was the *add* form, whose state died with the component on navigation. A row also showed only its URL and badges, so the events and the throttle interval were invisible — which is how a cn4m callback sat un-subscribed from `progress`, with the wrong URL, for two days without anyone being able to see it. Rows now show `events · progress at most every Ns` and carry **Edit**; the form fields are extracted into a shared `CallbackFields` used by both add and edit, because they had drifted once already (the add form grew a Discord option the non-existent edit form would not have had). A blank secret keeps the stored one — the backend already did that, `secret_encrypted = CASE WHEN ?='' THEN secret_encrypted ELSE ?`, and the field says so |
| D-139 | **Startup warns when the stored cn4m URL differs from `CN4M_CASCADE_STATUS_URL`** | The seed never overwrites (D-114), which is right — a variable in a compose file must not undo a change made in the UI — but it has a sharp edge that bit for real: a database **copied from the container deployment to the native one** carried the container's `host.docker.internal` URL, and setting the variable correctly afterwards did nothing. `host.docker.internal` still *resolves* natively, to an address Docker Desktop leaves behind — 192.168.1.36 here, which is none of this machine's addresses and times out — so it was not even a DNS failure. Deliveries are best-effort, so nothing errored and nothing logged: the suite view simply stayed empty. One warning at startup naming both URLs and saying the stored one wins is what turns a silent misconfiguration into something an operator can act on |
| D-140 | **cn4m progress posts go out at `level=progress`; start and outcome keep their own levels** | cn4m added the level 2026-09-12 specifically so a per-second update replaces itself in a per-app slot instead of stacking a dozen near-identical "Sync in Progress" rows into the feed per minute. Only `EventProgress` maps to it. `run_started` stays `working` because the start belongs in the feed and the log, and the outcome keeps `ok`/`warning`/`error` because **that post is what clears the slot** — an outcome sent as `progress` would leave the last percentage on the rail until cn4m's two-minute timeout and never reach the log. The slot times out after two minutes of silence; our cadence is the flush tick throttled to 5 s, on a timer rather than on file boundaries, so a single 100 GB file keeps the line alive. The vocabulary test gained the seventh level; a wire-level test sends start → progress → progress → outcome and checks each level on the receiving side |
| D-141 | **`notify` delivers each webhook's events through its own serial lane, not a shared worker pool** | The pool (four workers, one queue) reordered freely: two deliveries for one hook could be in flight at once, and whichever receiver answered first won. Under the feed protocol that was a row out of order. Under the slot protocol (D-140) a `progress` that lands after the run's `ok` re-fills the slot the `ok` just cleared, and "97%" stays up for two minutes after the sync finished. A lane per hook — a goroutine and a `queueDepth` channel, created on the hook's first event — makes order a property of the structure rather than of timing, keeps hooks parallel with each other, and isolates a hanging receiver to its own lane, which the pool also gave. Not a per-hook mutex around delivery: `sync.Mutex` is not FIFO, so waiters would still acquire in an unspecified order. Webhooks are operator-configured and few, so a goroutine each costs nothing; `Stop` sets a flag under the lock before waiting so no lane can start once `Wait` has begun |
...
**Update 2026-09-12 — the `progress` level.** cn4m gained a seventh level with different semantics
from the other six: `progress` goes to a **per-app slot** (one Redis hash field per app) rather than
the feed. Each post replaces the app's previous line; none of them reach the feed, the tray or the
log; the slot is cleared by the app's next non-progress post, or dropped after two minutes of
silence so a cascade that dies mid-sync does not leave "16%" on the rail for good. The rail paints it
in the working orange and polls every 2 s while one is live. It is the suite-side twin of cn4m's own
`set_app_progress()`.

`EventProgress` now maps to it (D-140). Everything else keeps its level, and that is not incidental:
`run_started` stays `working` so the start is a real feed event, and the outcome stays `ok` /
`warning` / `error` because that post is what *clears* the slot. Sending the outcome as `progress`
would leave "97%" up until the timeout and the finished sync would never reach the log.

**It exposed an ordering bug the old protocol tolerated.** `notify` ran four workers over one
shared queue, so two deliveries for the same hook could be in flight at once — and with a feed that
merely meant a row slightly out of order. With a slot it means a `progress` request that lands
*after* the run's `ok` re-fills the slot the `ok` just cleared. Delivery is now **one serial lane per
webhook** (D-141): a hook's events go out in queue order, hooks still run in parallel, and a hanging
receiver only holds up its own lane. Three tests pin it — the slot contract on the wire, forty
jittered deliveries arriving in sequence (a pool fails that reliably, not occasionally), and a
healthy hook not waiting behind a hanging one.
